...
namespace ai
{
    ...
    {
        auto it = DummyObject::m_propertiesMap.find(propName);
        if (it != DummyObject::m_propertiesMap.end())
        {
            return it->second;
        }
    ...
    bool DummyObject::SetPropertyById(int propertyId, m3d::AIParam const& newValue)
    {
        if (propertyId == 44)
        {
            SetModelNameUnsafe(newValue.GetAsStr());
            return 1;
        }
    ...
    {
        return RT_DYNCAST(thePrototypeManager->GetPrototypeInfo(GetPrototypeId()), DummyObjectPrototypeInfo const);
    }

    void DummyObject::SetModelNameUnsafe(CStr const& modelName)
    {
        m_modelName = modelName;
        if (m_physicBody)
        {
            m_physicBody->SetModelNameUnsafe(modelName);
            m_physicBody->ApplyCurrentModelCollision();
        }
        _UpdateCollisionInfoFromPhysicBody();
        SetMass(GetMass());
    }
