...
namespace ai
{
    namespace
    {
        Quest::SubQuestCondition Str2SubQuestCondition(CStr const& str)
        {
            if (str == "and")
            {
        ...
        void Str2ConditionToGive(CStr const& str, Quest::ConditionToGive& condition)
        {
            if (str.empty())
            {
    ...
    Quest::ConditionToGive::ConditionToGive(ConditionOperator op, ConditionType typ) : m_operator(op), m_type(typ)
    {
    }
    ...
    bool QuestItemPrototypeInfo::LoadFromXML(m3d::cmn::XmlFile* xmlFile, m3d::cmn::XmlNode const* xmlNode)
    {
