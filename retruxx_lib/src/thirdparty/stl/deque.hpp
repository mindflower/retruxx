...
public:
	...
	void swap(_Myt& _Right)
		...
		if (this->_Alval == _Right._Alval)
			...
			oldstd::swap(_Map, _Right._Map);
			oldstd::swap(_Mapsize, _Right._Mapsize);
			oldstd::swap(_Myoff, _Right._Myoff);
...
protected:
	...
	void _Xlen() const
		{	// report length error
		_THROWOLD(length_error, "deque<T> too long");
		}

	void _Xran() const
		{	// report range error
		_THROWOLD(out_of_range, "invalid deque<T> subscript");
		}
